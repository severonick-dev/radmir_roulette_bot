...
# ---------------------------------------------------------------------------
async def _prediction_text(ai, result, data: dict, window: int) -> str:
    if ai is not None:
        try:
            return await ai_report.predict_next(
                ai, result, server=data["server"], casino=data["casino"],
                table_no=data["table"], window=window,
            )
        except Exception:
            logging.exception("AI predict failed")
    return stats.predict_local(result)
...
async def _process_spin(message: Message, state: FSMContext, number: int, ai, window: int) -> None:
    ...
    prediction = await _prediction_text(ai, result, data, window)
    color = domain.classify(number).color
    text = texts.spin_full(number, color, total, prediction)
    try:
